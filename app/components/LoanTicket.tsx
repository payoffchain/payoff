 ...
 */
export default function LoanTicket({ symbol = "NVDA", price = 224.72, apy = 0.0003 }: { symbol?: string; price?: number; apy?: number }) {
  const collateral = 10;
  ...
  return (
    <div className="ticket" aria-label="Worked example of a self-repaying loan">
      ...
      <div className="tk-rows">
        ...
        <div><span>BORROWED</span><b>{usd(borrowed, 0)} USDG</b></div>
        <div><span>RATE</span><b>{(apy * 100).toFixed(2)}% <small>/ YR</small></b></div>
        <div><span>DEBT NOW</span><b className="tk-debt">{usd(debt)}</b></div>
      </div>
      <div className="tk-sub"><span>FEES COLLECTED</span><span>USDG → DEBT</span></div>
