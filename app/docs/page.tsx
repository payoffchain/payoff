    ...
    <>
      ...
      <main className="wrap" style={{ padding: "48px 24px 80px", maxWidth: 820 }}>
        ...
        <div className="card soft" style={{ marginTop: 14 }}>
          ...
          <div className="kv"><span>Neither</span><b>send a token to any address but the lending market, the position manager, the swap router, or the treasury's capped fee</b></div>
        </div>
        ...
        <div className="card soft" style={{ marginTop: 14 }}>
          <div className="kv"><span>Your collateral (the stock)</span><b>Held by the lending market under your vault's name. Comes back with <em>Withdraw collateral</em> on the vault page, straight to your wallet.</b></div>
        ...
        <div className="steps" style={{ marginTop: 14 }}>
          <div className="step"><div><h3>Who can move it</h3><p>The vault contract only ever sends tokens to four places: the lending market, the Uniswap position manager, the Uniswap router, and your own wallet. Your wallet is the only destination for a withdrawal. The agent's key cannot change that, and neither can {APP}. This is in the code, not in a promise.</p></div></div>
          <div className="step"><div><h3>How to get everything out</h3><p>Two or three clicks on the vault page, in this order: <em>Close</em> any open position into USDG (the agent may have done this already), <em>Repay</em> the debt, then <em>Withdraw collateral</em>. If USDG is left over after repaying, <em>Withdraw token</em> sends it to you. The market will not let collateral leave while it still backs debt, which is why repay comes before withdraw.</p></div></div>
          <div className="step"><div><h3>What can make it smaller</h3><p>Three things, all visible on the vault page: interest on the USDG you borrowed (the rate shown on the market), the market moving against a liquidity position (a position that holds more of the stock after the price fell is worth less in USDG), and, if the price falls far enough with nobody repaying, liquidation. The policy's protection trigger exists to repay before that last one happens; it only works while an agent is running.</p></div></div>
        ...
        <p className="mute" style={{ marginTop: 14 }}>Every official address, with links to the verified source code, is listed on the <Link href="/contracts" style={{ textDecoration: "underline" }}>contracts page</Link>.</p>
        ...
        <div className="steps" style={{ marginTop: 10 }}>
          ...
          <div className="step"><div><h3>Protection trigger and repay share</h3><p>Once LTV reaches the trigger, <code className="inline">protect()</code> repays the given share of the debt: first with idle USDG, then by closing positions into USDG, then by selling collateral. That last step borrows the repayment from the lending market's flash loan, frees the collateral, sells it, and pays the loan back inside one transaction.</p></div></div>
          <div className="step"><div><h3>Max slippage</h3><p>Every swap the vault makes gets a floor of the market oracle price less this. Every mint and burn first checks that the pool's spot price is within this of the oracle. An operator can pass a floor of zero; the vault tightens it.</p></div></div>
        </div>

        <h3 style={{ marginTop: 40 }}>Refinancing</h3>
        <p className="mute">On {CHAIN_NAME} one collateral usually has several lending markets against USDG, at different LLTVs and with different curators. Each has its own utilization and so its own borrow rate. You allow-list the markets you trust; when one is cheaper by at least the savings threshold, has the liquidity, and keeps LTV inside the ceiling, the agent moves the whole position there: flash-borrow the debt, repay the old market, withdraw the collateral, supply it to the new market, borrow there, repay the flash loan. All or nothing.</p>
        ...
        <div className="card soft" style={{ marginTop: 10 }}>
          ...
          <div className="kv"><span>Gas and protocol fees</span><b>standard, paid by the vault or the operator</b></div>
        </div>
...
GET  /api/markets[?stocks=1]                 USDG lending markets by collateral, live rates
