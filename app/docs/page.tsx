  ...
  return (
    <>
      ...
      <main className="wrap" style={{ padding: "48px 24px 80px", maxWidth: 820 }}>
        ...
        <h2>How {APP} works.</h2>
        <p className="lede">{APP} is a non-custodial protocol for self-repaying loans on {CHAIN_NAME}. You borrow USDC against your BTC (cirBTC) or ETH on Morpho Blue; the borrowed USDC earns Uniswap V3 trading fees and every fee is paid onto the debt. The part that does the work is a small program we call the agent; this page explains it in full.</p>

        <h3 style={{ marginTop: 40 }}>The vault</h3>
        <p className="mute">Every position lives in its own <code className="inline">PayoffVault</code>, a small contract the factory clones for you. It holds one pair — collateral token and loan token — for its whole life. The collateral sits in a Morpho market under the vault's name; borrowed USDC stays in the vault until it is deployed into the pair's Uniswap V3 pool as an NFT position the vault owns.</p>
        ...
        <div className="card soft" style={{ marginTop: 14 }}>
          <div className="kv"><span>Your collateral (cirBTC, WETH…)</span><b>Held by Morpho under your vault's name. Comes back with <em>Withdraw collateral</em> on the vault page, straight to your wallet.</b></div>
          <div className="kv"><span>The USDC you borrowed</span><b>Sits in the vault until it is put into the Uniswap pool. Idle USDC comes back with <em>Withdraw token</em>; USDC in a pool comes back when the position is closed.</b></div>
        ...
        <div className="steps" style={{ marginTop: 14 }}>
          ...
          <div className="step"><div><h3>How to get everything out</h3><p>Two or three clicks on the vault page, in this order: <em>Close</em> any open position into USDC (the agent may have done this already), <em>Repay</em> the debt, then <em>Withdraw collateral</em>. If USDC is left over after repaying, <em>Withdraw token</em> sends it to you. Morpho will not let collateral leave while it still backs debt, which is why repay comes before withdraw.</p></div></div>
          <div className="step"><div><h3>What can make it smaller</h3><p>Three things, all visible on the vault page: interest on the USDC you borrowed (the rate shown on the market), the market moving against a liquidity position (a position that holds more of the stock after the price fell is worth less in USDC), and, if the price falls far enough with nobody repaying, liquidation by Morpho. The policy's protection trigger exists to repay before that last one happens; it only works while an agent is running.</p></div></div>
        ...
        <p className="mute">On {CHAIN_NAME} one collateral usually has several Morpho markets against USDC, at different LLTVs and with different curators. Each has its own utilisation and so its own borrow rate. You allow-list the markets you trust; when one is cheaper by at least the savings threshold, has the liquidity, and keeps LTV inside the ceiling, the agent moves the whole position there: flash-borrow the debt, repay the old market, withdraw the collateral, supply it to the new market, borrow there, repay the flash loan. All or nothing.</p>
        ...
        <p className="mute">The brain is one file, <code className="inline">lib/services/plan.ts</code>, evaluated by the site for any vault at <code className="inline">/api/vaults/&lt;vault&gt;/plan</code>. In priority order: protect, refinance, close (loss limit or out of range), harvest (above the fee floor), deploy idle USDC into the pool with the best recent fee yield. The runner (<code className="inline">agent/index.mjs</code>) fetches the plan every tick, simulates each transaction, and signs with the operator key. It starts in dry-run. With an Anthropic key, Claude may veto an action with a reason — it can never add one.</p>
        ...
        <ul className="mute" style={{ paddingLeft: 20, lineHeight: 1.7 }}>
          <li>A concentrated position carries impermanent loss; if the price moves out of the range it stops earning and the agent closes it into USDC, realising the move.</li>
          ...
          <li>The vault refuses to mint or burn while the pool price drifts from the Morpho oracle beyond your slippage; if an oracle ever stops answering, it refuses to trade at all rather than guess.</li>
...
GET  /api/markets[?stocks=1]                 Morpho USDC markets by collateral, live rates (stocks=1: featured collateral only)
