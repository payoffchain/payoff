...
/** cirBTC on Arc: the collateral the site leads with (deepest Morpho market, live Uniswap pool). */
const LEAD = "0x171A4217b86A807A64eB94757Db6849fb4bDbAA0";
...
export default function Landing() {
  ...
  // The busiest collateral pool on the chain, for the statement band. cirBTC/USDC today; the
  // symbol shown comes from the API so a change of token needs only this address.
  const pools = useLive<Pools | null>(`/api/pools?collateral=${LEAD}&hours=6`, null);
  ...
  const lead = withRate.find((g) => g.collateral.address.toLowerCase() === LEAD.toLowerCase()) ?? null;
      ...
      <main>
        <section className="hero">
          <div className="wrap hero-grid">
            <div>
              <div className="hero-logo" style={{ animation: "fadeUp 1s .2s both" }} onMouseMove={tilt} onMouseLeave={untilt}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src="/mark.png" alt={APP} width={512} height={512} /></div>
              <span className="chip"><span className="dot" />loan desk open</span>
              <h1>
                <Words text="Borrow against your BTC." base={80} />
                <br />
              ...
              <p className="lede" style={{ animation: "fadeUp .8s .9s both" }}>
                Put your BTC or ETH in as collateral, borrow USDC against it, and let the loan repay itself: the borrowed USDC earns trading fees in a Uniswap pool, and every fee goes onto your debt. Only you can take money out.
              </p>
            ...
            <div>
              <LiveLoan symbol={lead?.collateral.symbol ?? "cirBTC"} price={lead?.best?.collateralPrice ?? 114_000} apy={lead?.best?.borrowApy ?? 0.0003} />
            </div>
          </div>
        </section>
          ...
          <div className="track">
            {[0, 1].map((k) => (
              <span key={k} style={{ display: "inline-flex", gap: 40 }}>
                ...
                )) : <span>{board.loading ? "reading Morpho markets on Arc…" : "Morpho rates unavailable right now"}</span>}
              </span>
            ))}
          </div>
        </div>
        ...
        <section>
          <div className="wrap band">
            <Reveal>
              <div className="huge">{vol ? usd(vol.volumeLoan) : "0%"}<small>{vol ? `traded in the ${vol.symbol}/USDC pool in the last ${vol.hours}h` : "to borrow. 0% to move markets. 0% to be protected."}</small></div>
            </Reveal>
            <Reveal delay={120}>
              <h2>{vol ? "Every one of those swaps paid a fee. That fee is what repays your loan." : "You pay nothing until the loan earns."}</h2>
              <p style={{ marginTop: 14 }}>
                {vol
                  ? <>BTC on {CHAIN_NAME} trades around the clock, and the Uniswap pool takes <b>{(vol.fee / 10_000).toFixed(2)}%</b> of every swap for the people who put liquidity in. Your borrowed USDC becomes that liquidity. Its fees go straight onto your debt, every time they are collected, without you doing anything.</>
          ...
          <div className="wrap">
            <div className="row" style={{ justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16 }}>
              <div>
                ...
                <h2 style={{ marginTop: 10 }}>Pick your collateral. Borrow USDC against it.</h2>
              </div>
                ...
                <Reveal key={g.collateral.address} delay={(i % 4) * 60} as={Link} className={"mk" + (i === 0 && !q ? " featured" : "")} href={`/borrow?market=${g.best!.id}`}>
                  <div className="top">
                    ...
                    <span className="id">
                      ...
                      <span className="name">{g.collateral.name ?? (g.collateral.isStock ? "featured collateral" : "collateral")}</span>
                    </span>
                    <span className={"tag " + (g.collateral.isStock ? "stock" : "")}>{g.collateral.isStock ? "featured" : "other"}</span>
                  </div>
                  ...
                  <div className="cta"><span className="btn green go">Borrow USDC<span className="arr">→</span></span></div>
                </Reveal>
              ))}
              {shown.length === 0 && (
        ...
        <section>
          <div className="wrap">
            <Reveal><span className="eyebrow">Demo</span><h2>See it run, start to finish.</h2><p className="lede">Connect, pick your collateral, choose how careful to be, open the loan, and watch it put the USDC to work, collect fees onto the debt and move to a cheaper market. Click to pause.</p></Reveal>
        ...
        <section>
          <div className="wrap">
            ...
            <Reveal delay={100} style={{ marginTop: 28, maxWidth: 640 }}><LoopDiagram symbol={lead?.collateral.symbol ?? "cirBTC"} /></Reveal>
            <div className="desk3">
              {[
                { t: "deposit", n: "01", href: "/borrow", cta: "Open a loan ↗", p: "Put cirBTC or WETH into a vault contract only you own. It sits in a Morpho market under your vault's name and can leave only to your wallet." },
                { t: "borrow", n: "02", href: "/rates", cta: "See the rates ↗", p: "Borrow USDC against it, up to a ceiling you set. The USDC goes straight into the Uniswap pool for that asset, in a range around today's price." },
            ...
            <div className="timeline" hidden>
              {[
                { t: "Collateral in, USDC out", p: `Your cirBTC or WETH goes into a Morpho Blue market under a vault contract only you own. You borrow USDC up to a ceiling you set; Morpho's liquidation line is further out.` },
                { t: "The loan goes to work", p: `Your USDC goes into the Uniswap V3 pool for that asset, in a range around today's price. Every swap in the pool pays the range a fee.` },
                { t: "Fees pay the debt", p: `Every fee collected goes straight onto the loan. A cheaper market for the same collateral? The debt moves there in one transaction. Price falling toward your safety line? Part of the loan is repaid before Morpho could ever liquidate.` },
              ].map((s, i) => (
          ...
          <div className="wrap">
            ...
            <div className="bento" style={{ marginTop: 32 }}>
              ...
              <Reveal className="card feature b-3" delay={100}>
                ...
                <p>The part that repays your loan may put USDC into the pool, collect fees onto the debt, move the debt to a cheaper market, and repay early. It cannot send a token to any address. Even if its key leaked, nobody could steal from you.</p>
              </Reveal>
              ...
              <Reveal className="card feature b-2" delay={200}><div className="ico">{I.life}</div><h3>Liquidation protection</h3><p>Choose a safety line below where Morpho would liquidate. When the price gets there, part of the loan is repaid: from idle USDC first, then the pool position, and only then a slice of collateral.</p></Reveal>
        ...
        {FACTORY && (
          <section style={{ padding: "40px 0 72px" }}>
            <div className="wrap stats">
              ...
              <Reveal className="stat" delay={80}><span className="lbl">Debt outstanding</span><span className="big"><CountUp value={lb.data?.totals.debtUsd ?? 0} format={(n) => usd(n)} /></span><span className="faint" style={{ fontSize: 12 }}>USDC across all vaults</span></Reveal>
        ...
        <section>
          <div className="wrap">
            <Reveal className="cta">
              ...
              <h2>Put your idle BTC to work.</h2>
